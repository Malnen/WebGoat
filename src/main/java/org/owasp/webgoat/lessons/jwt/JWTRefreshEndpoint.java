...
public class JWTRefreshEndpoint extends AssignmentEndpoint {
    ...
    @Value("jerry.password")
    private static String JERRY_PASSWORD;
    ...
    public ResponseEntity follow(@RequestBody(required = false) Map<String, Object> json) {
        ...
        if ("Jerry".equalsIgnoreCase(user) && JWT_PASSWORD.equals(password)) {
    ...
    public ResponseEntity<AttackResult> checkout(
        ...
        try {
            Jwt jwt = Jwts.parser().setSigningKey(JWT2_PASSWORD).parse(token.replace("Bearer ", ""));
    ...
    public ResponseEntity newToken(
        ...
        try {
            Jwt<Header, Claims> jwt =
                    Jwts.parser().setSigningKey(JWT2_PASSWORD).parse(token.replace("Bearer ", ""));
